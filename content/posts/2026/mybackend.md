...
### 修改密码：枚举攻击（Enumeration Attack）

应该先算新旧密码是否一致

如果先算旧密码是否匹配数据库，输入一个错的会返回401，如果碰到输入正确，会返回400，规律就是，只要返回401，说明我试的旧密码是错的；一旦返回400，我就知道我试出来的这个旧密码一定是数据库里那个真实的密码

而如果先算新旧密码是否匹配，两者相同，直接返回400，无法判断输入密码是否真的正确，阻止猜出真的密码。

以及统一错误码和报错提示，模糊化错误提示是低成本、高回报的安全加固。

（待续）

虽然仓库还很简陋，虽然代码都没什么难点，虽然我还在填以前的坑，但至少每一行代码都是我亲自敲出来的（tab也算的话）。这种对代码的掌控感，是任何 AI Agent 都无法提供的温存。

2026-1-2：我错了，qodo就是来专门找茬的，为了纠结一个无所吊谓的小问题反复折磨我

感觉好像能写的很多，有很多问题不确定值不值得记下来
